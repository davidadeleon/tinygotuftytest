//go:generate pioasm -o go st7789_parallel.pio st7789_parallel_pio.go
package main
...
func (st *ST7789) writeBlockingDMA(data []byte, length int) {
	...
	for machine.DMAChannels[st.dmaChannel].CTRL_TRIG.Get()&rp.DMA_CH0_CTRL_TRIG_BUSY != 0 {
		//noop
	}
	...
	machine.DMAChannels[st.dmaChannel].TRANS_COUNT.Set(uint32(length))
	machine.DMAChannels[st.dmaChannel].READ_ADDR.Set(uint32(readAddr))
	machine.DMAChannels[st.dmaChannel].CTRL_TRIG.Set(machine.DMAChannels[st.dmaChannel].CTRL_TRIG.Get() | rp.DMA_CH0_CTRL_TRIG_EN)
}
